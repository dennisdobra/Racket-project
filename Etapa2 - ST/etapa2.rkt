...
;; 1. obținem toate sufixele textului - TODO 1
...
(define (get-ch-words words ch)
  (filter (lambda (x) (char=? (car x) ch)) words))
...
(define (cst-func suffixes)
  (cons (longest-common-prefix-of-list suffixes)
        (map (lambda (x) (drop x (length (longest-common-prefix-of-list suffixes)))) suffixes)))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (if (null? alphabet)
      ...
      null
      ;; else:
      (let ((generate-symbol-branch (lambda (list-of-suffixes alfabet) ; generate-symbol-branch este o variabila locala definita prin functia anonima care construieste pe rand fiecare ramura din st ca pereche intre: 
                                      (if (null? list-of-suffixes)
                                          null
                                          (cons
                                           ...
                                           [filter (lambda (x) (not (null? x))) (suffixes->st labeling-func (filter (lambda (x) (not (null? x))) (cdr (labeling-func list-of-suffixes))) alfabet)]))))) ; si subarborele de sub eticheta
        (filter
         (lambda (x) (not (null? x))) ; predicatul
         (map ; aplica pentru fiecare litera din alfabet functia anonima lambda care primeste un parametru, symbol, pentru care se apeleaza functia de mai sus, generate-symbol-branch, cu param (sufixele care incep cu litera symbol) si (alphabet)
...
; asociat textului.(define (text->cst text)
