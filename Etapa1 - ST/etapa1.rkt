...
(define (longest-common-prefix-helper prefix rest-w1 rest-w2)
  (cond
    ...
    [(equal? (car rest-w1) (car rest-w2))
     (longest-common-prefix-helper (cons (car rest-w1) prefix) (cdr rest-w1) (cdr rest-w2))] ; prefixul este construit in ordine inversa
    [else
  ...
  (let ([label (and (not (st-empty? st)) (get-branch-label (first-branch st)))]
    ...
    (cond
      ...
      [(equal? (car (longest-common-prefix label pattern)) label)
       (list label (caddr (longest-common-prefix label pattern)) subtree)] ; mai pot cauta in subarbore
      [(not (null? (car (longest-common-prefix label pattern))))
       (list #f (car (longest-common-prefix label pattern)))] ; lista formata din #f si cel mai lung prefix comun
      [else
       (match-pattern-with-label (other-branches st) pattern)])))
...
; cddr value = subarborele in care pot continua cautarea
; cadr value = noul sablon care trb cautat